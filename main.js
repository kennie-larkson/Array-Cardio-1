// Get your shorts on - this is an array workout!
    ...
    const inventors = [
      ...
      const filterAge = inventors.filter(inventor => inventor.year >= 1500 && inventor.year < 1600);
      ...
      //or const fullName = inventors.map(inventor => `${inventor.first}  ${inventor.last}`);
      ...
      /* the .reduce() method tries to do away with having to initialize a storage variable as below
      var total = 0;
      ...
      const links = Array.from(category.querySelectorAll('a')) ;//converting NodeList into array so it can be 'mapped'
      const de = links
